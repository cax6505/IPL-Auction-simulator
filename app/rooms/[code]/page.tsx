...
export default function RoomLobbyPage() {
  ...
  useEffect(() => {
    if (!loading && room) {
      if (room.status === "active" || room.status === "in_progress" || room.status === "paused") {
  ...
  return (
    <div className="min-h-screen surface-0 text-slate-300 font-sans flex flex-col">
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 flex flex-col gap-6 pb-28 md:pb-8">
        <AuctionLobby />
      </main>
    </div>
  );
